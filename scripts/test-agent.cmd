...
if exist "%REPO_ROOT%\.tooling\dotnet8\dotnet.exe" (
  "%REPO_ROOT%\.tooling\dotnet8\dotnet.exe" run --project "%REPO_ROOT%\tests\Agent.Tests\Agent.Tests.csproj" --no-build
) else (
  dotnet run --project "%REPO_ROOT%\tests\Agent.Tests\Agent.Tests.csproj"
)
