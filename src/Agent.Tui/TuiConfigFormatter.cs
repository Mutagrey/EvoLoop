...
internal static class TuiConfigFormatter
{
    ...
    private static void AppendHeader(TuiRuntimeInfo runtime, StringBuilder sb)
    {
        ...
        sb.AppendLine($"runtime mode: {runtime.ModeLabel}");
        sb.AppendLine($"auth: {(runtime.ApiAuthConfigured ? "present" : "missing")}; gateway: {(runtime.ModelReachable ? "reachable" : "unreachable")}");
        sb.AppendLine($"active model profile: {runtime.Profile}; model: {runtime.ModelId}");
        sb.AppendLine();
    }
    ...
    private static void AppendPrompts(TuiRuntimeInfo runtime, StringBuilder sb)
    {
        ...
        foreach (var path in runtime.PromptPaths)
        {
            sb.AppendLine($"  {path}");
        }
        sb.AppendLine();
